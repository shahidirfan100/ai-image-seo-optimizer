...
final class AIISO_Admin {
    public static function init(): void {
        ...
        // Belt-and-suspenders fallback: some local/reverse-proxy/cache setups can serve
        // plugin PHP while failing to serve plugin asset URLs. Print the bundled admin
        // assets directly on our own screens so the UI never degrades into raw HTML.
        add_action( 'admin_head', array( __CLASS__, 'inline_css_fallback' ), 99 );
        add_action( 'admin_print_footer_scripts', array( __CLASS__, 'inline_js_fallback' ), 999 );
    ...
    public static function assets( string $hook ): void {
        ...
        $css_path = AIISO_DIR . 'assets/admin.css';
        $js_path  = AIISO_DIR . 'assets/admin.js';
        $css_ver  = file_exists( $css_path ) ? (string) filemtime( $css_path ) : AIISO_VERSION;
        $js_ver   = file_exists( $js_path ) ? (string) filemtime( $js_path ) : AIISO_VERSION;

        wp_enqueue_style( 'dashicons' );
        wp_enqueue_style( 'aiiso-admin', AIISO_URL . 'assets/admin.css', array(), $css_ver );
        wp_enqueue_script( 'aiiso-admin', AIISO_URL . 'assets/admin.js', array( 'jquery' ), $js_ver, true );
    ...
    /**
     * Direct CSS fallback for WordPress installs where a plugin asset URL is blocked,
     * stale, rewritten, or unavailable. The stylesheet is a bundled trusted file.
     */
    public static function inline_css_fallback(): void {
        if ( ! self::is_plugin_page() ) { return; }
        $path = AIISO_DIR . 'assets/admin.css';
        ...
        echo "\n<style id=\"aiiso-admin-inline-fallback\">\n" . $css . "\n</style>\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }

    /**
     * Direct JS fallback paired with inline_css_fallback(). admin.js has a global
     * execution guard, so it is safe when the normal enqueued script also succeeds.
     */
    public static function inline_js_fallback(): void {
        if ( ! self::is_plugin_page() ) { return; }
        $path = AIISO_DIR . 'assets/admin.js';
        if ( ! is_readable( $path ) ) { return; }
        $js = file_get_contents( $path ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
        if ( false === $js || '' === trim( $js ) ) { return; }
        $config = wp_json_encode( array(
            'ajax'    => admin_url( 'admin-ajax.php' ),
            'nonce'   => wp_create_nonce( 'aiiso_ajax' ),
            'version' => AIISO_VERSION,
        ) );
        echo "\n<script id=\"aiiso-admin-inline-config\">window.AIISO=window.AIISO||" . $config . ";</script>\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        echo "<script id=\"aiiso-admin-inline-fallback\">\n" . $js . "\n</script>\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }

    private static function render_runtime_assets(): void {
        ...
        echo "\n<script id=\"aiiso-runtime-config\">window.AIISO=" . $config . ";</script>\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

        $js_path = AIISO_DIR . 'assets/admin.js';
        if ( is_readable( $js_path ) ) {
            $js = file_get_contents( $js_path ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
            if ( false !== $js && '' !== trim( $js ) ) {
                echo "<script id=\"aiiso-runtime-js\">\n" . $js . "\n</script>\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            }
        }
    }
    ...
    private static function app_header( string $current, string $title, string $subtitle, string $action_html = '' ): void {
        ...
        <div class="aiiso-app-head">
            <div class="aiiso-brand-row">
                <div class="aiiso-brand-mark" aria-hidden="true"><span class="dashicons dashicons-format-image"></span></div>
    ...
    public static function save_attachment_fields( array $post, array $attachment ): array {
        if ( isset( $attachment['aiiso_decorative'] ) ) {
            update_post_meta( $post['ID'], '_aiiso_decorative', 1 );
            update_post_meta( $post['ID'], '_wp_attachment_image_alt', '' );
        } else {
            delete_post_meta( $post['ID'], '_aiiso_decorative' );
        }
        return $post;
    }
