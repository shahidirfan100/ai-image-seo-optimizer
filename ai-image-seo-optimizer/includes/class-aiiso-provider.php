...
final class AIISO_Provider {
    ...
    public static function test( string $provider, string $key_override = '', string $model_override = '' ) {
        ...
        // Use the bundled brand mark so the test confirms that the selected model
        // accepts real image input rather than validating text chat only.
        $prepared_test_image = self::prepare_image( AIISO_DIR . 'assets/images/logo-admin.png', 96 );
        if ( is_wp_error( $prepared_test_image ) ) { return $prepared_test_image; }
        [ $test_bytes, $test_mime ] = $prepared_test_image;
        $test_image = 'data:' . $test_mime . ';base64,' . base64_encode( $test_bytes );
        $payload = array(
            'model' => $model,
            'messages' => array(
                array(
